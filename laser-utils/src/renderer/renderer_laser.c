...
static void renderer_laser_draw(BotViewer *viewer, BotRenderer *renderer)
{
  ...
  for (int chan_idx = 0; chan_idx < self->channels->len; chan_idx++) {
    ...
    for (int scan_idx = 0; scan_idx < scan_count; scan_idx++) {
      ...
      numPointsToDraw += lscan->numValidPoints;
    }
  }
  ...
  if (numPointsToDraw > self->pointBuffSize) {
    ...
    self->colorBuffer = (float *) realloc(self->colorBuffer, numPointsToDraw * 3 * sizeof(float));
  }
    ...
    for (int scan_idx = 0; scan_idx < scan_count; scan_idx++) {
      ...
      for (int i = 0; i < lscan->npoints; i++) {
        if (lscan->invalidPoints[i]) {
          ...
          float * colorV = self->colorBuffer + (3 * numLoadedPoints); //pointer into the color buffer
          switch (color_mode) {
          case COLOR_MODE_Z:
            {
              double z;
